...
function GameEvents() {
    ...
    $('#startBtn').on('click', function() {
        if (uploadedImage && !gameStarted) { // 이미지가 있고 게임이 시작되지 않았으면
...
function DragAndDrop() { 
    ...
    // 업로드 영역 드래그 앤 드롭
    uploadArea.addEventListener('dragover', function(e) { // 드래그 오버 이벤트
        e.preventDefault(); // 기본 동작 방지
        e.stopPropagation(); // 이벤트 전파 방지
        uploadArea.classList.add('drag-over'); // 드래그 오버 클래스 추가
    });
    
    uploadArea.addEventListener('dragleave', function(e) { // 드래그 리브 이벤트
        e.preventDefault(); // 기본 동작 방지
        e.stopPropagation(); // 이벤트 전파 방지
        if (!uploadArea.contains(e.relatedTarget)) { // 관련 타겟이 업로드 영역 밖이면
            uploadArea.classList.remove('drag-over'); // 드래그 오버 클래스 제거
        }
    });
    
    uploadArea.addEventListener('drop', function(e) { // 드롭 이벤트
        e.preventDefault(); // 기본 동작 방지
        e.stopPropagation(); // 이벤트 전파 방지
        uploadArea.classList.remove('drag-over'); // 드래그 오버 클래스 제거
    ...
    // 캔버스 영역 드래그 앤 드롭
    if (canvasContainer) { // 캔버스 컨테이너가 있으면(이미 파일이 올라간 상태여도 바로 수정할 수 있게)
        canvasContainer.addEventListener('dragover', function(e) { // 드래그 오버 이벤트
            e.preventDefault(); // 기본 동작 방지
            e.stopPropagation(); // 이벤트 전파 방지
            canvasContainer.classList.add('drag-over'); // 드래그 오버 클래스 추가
        });
        
        canvasContainer.addEventListener('dragleave', function(e) { // 드래그 리브 이벤트
            e.preventDefault(); // 기본 동작 방지
            e.stopPropagation(); // 이벤트 전파 방지
            if (!canvasContainer.contains(e.relatedTarget)) { // 관련 타겟이 컨테이너 밖이면
                canvasContainer.classList.remove('drag-over'); // 드래그 오버 클래스 제거
            }
        });
        
        canvasContainer.addEventListener('drop', function(e) { // 드롭 이벤트
            e.preventDefault(); // 기본 동작 방지
            e.stopPropagation(); // 이벤트 전파 방지
            canvasContainer.classList.remove('drag-over'); // 드래그 오버 클래스 제거
    ...
    // 문서 전체 드래그 방지
    document.addEventListener('dragover', function(e) { // 문서 전체 드래그 오버 이벤트
        e.preventDefault(); // 기본 동작 방지
    });
    
    document.addEventListener('drop', function(e) { // 문서 전체 드롭 이벤트
        e.preventDefault(); // 기본 동작 방지
    });
}
...
function handleImageUpload(file) {
    if (!file.type.startsWith('image/')) { // 이미지 파일이 아니면
        alert('이미지 파일만 업로드 가능합니다.'); // 경고 메시지 표시
        return;
    }
    ...
    reader.onload = function(e) { // 파일 로드 완료 시 실행
        const img = new Image(); // 이미지 객체 생성
        img.onload = function() { // 이미지 로드 완료 시 실행
...
function drawImageOnCanvas(img) { 
    ...
    ctx.drawImage(
        img, // 그릴 이미지
        cropInfo.sourceX, cropInfo.sourceY, cropInfo.sourceWidth, cropInfo.sourceHeight, // 소스 영역
        0, 0, cropInfo.destWidth, cropInfo.destHeight // 대상 영역
    );
...
function calculateCropSize(imgWidth, imgHeight, canvasSize) { 
    ...
    return { // 크롭 정보 반환
        ...
        destWidth: canvasSize, destHeight: canvasSize // 대상 영역
    };
}
    ...
    for (let row = 0; row < 3; row++) { // 3행 반복
        for (let col = 0; col < 3; col++) { // 3열 반복
            ...
            ctx.drawImage(
                ...
                col * sourceSize, row * sourceSize, sourceSize, sourceSize, // 소스 영역
                0, 0, pieceSize, pieceSize // 대상 영역
            );
            ...
            puzzlePieces.push({ // 퍼즐 조각 객체를 배열에 추가
...
function placePiecesRandomly() {
    ...
    // 조각들을 섞기
    shuffleArray(puzzlePieces); // 퍼즐 조각 배열 섞기

    puzzlePieces.forEach(function(piece) { // 각 퍼즐 조각에 대해 반복
        let randomX, randomY; // 랜덤 X, Y 위치 변수
        
        if (Math.random() < 0.5) { // 50% 확률로
        ...
        $(piece.element).css({ // 조각 요소 스타일 설정
...
function shuffleArray(array) { 
    for (let i = array.length - 1; i > 0; i--) { // 배열 끝부터 시작해서
...
function startTimer() { 
    ...
    gameTimer = setInterval(function() { // 1초마다 실행
...
function updateTimerDisplay() {
    ...
    const timeString = minutes.toString().padStart(2, '0') + ':' + seconds.toString().padStart(2, '0'); // 시간 문자열 생성
    $('#timerDisplay').text(timeString); // 타이머 표시 업데이트
}

// 타이머 정지
function stopTimer() {
    if (gameTimer) { // 타이머가 있으면
...
function startDrag(e) { 
    ...
    if (!piece || piece.isCorrect) return; // 조각이 없거나 이미 올바른 위치에 있으면 종료
    
    draggedPiece = piece; // 드래그 중인 조각 설정
    
    const rect = e.target.getBoundingClientRect(); // 요소의 위치 정보 가져오기
    piece.dragOffset = { // 드래그 오프셋 설정
...
function handleDrag(e) {
    ...
    let newX = e.clientX - gameAreaRect.left - draggedPiece.dragOffset.x; // 새로운 X 위치 계산
...
function handleDragEnd(e) {
    ...
    if (checkCorrectPosition(draggedPiece)) { // 올바른 위치에 있는지 확인
...
function showHint() {
    ...
    const hintImg = $('<img class="hint-image" src="' + canvas.toDataURL() + '" alt="힌트 이미지">'); // 힌트 이미지 생성
    ...
    setTimeout(function() { // 3초 후 실행
        overlay.fadeOut(300, function() { // 페이드 아웃 애니메이션
            overlay.remove(); // 오버레이 제거
        });
    }, 3000); // 3초 대기
    
    // 클릭으로도 닫기 가능
    overlay.on('click', function() { // 오버레이 클릭 이벤트
        overlay.fadeOut(300, function() { // 페이드 아웃 애니메이션
            overlay.remove(); // 오버레이 제거
        });
    });
...
function showCompletionMessage(timeString) {
    ...
    restartBtn.on('click', function() { // 다시 시작 버튼 클릭 이벤트
        overlay.remove(); // 오버레이 제거
        resetGame(); // 게임 리셋 함수 호출
    });
...
function formatTime(seconds) {
    ...
    return minutes.toString().padStart(2, '0') + ':' + remainingSeconds.toString().padStart(2, '0'); // 시간 문자열 반환
}
